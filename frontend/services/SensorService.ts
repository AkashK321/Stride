...
// the object alignment check returns for UI to know what to display
export interface HeadingAlignmentResult {
    isAligned: boolean; // true when user is facing close enough to target
    degreesOff: number; // absolute angular difference (always 0–180)
    turnDirection: 'left' | 'right' | 'none'; // which way to turn
}
...
class SensorService {
    ...
    // Heading rolling average state

    // Note: this buffer holds raw expo-location heading values (added in index.tsx),
    // NOT the magnetometer heading. expo-location gives tilt-independent compass heading.
    private headingBuffer: number[] = [];

    // how many samples to average over - more samples = smoother but slower
    private readonly HEADING_WINDOW_SIZE = 5; 
    ...
    public readonly ALIGNMENT_TOLERANCE_DEGREES = 20; // Public readonly for DEV
    // This is the +-degrees window where the user is considered "aligned".
    // Starting with 20, we can tweak it later
    ...
    // index.tsx calls this every time expo-location gives us a new
    // compass heading. It feeds the rolling average buffer.
    public pushHeading(degrees: number) {
        this.headingBuffer.push(degrees);

        if(this.headingBuffer.length > this.HEADING_WINDOW_SIZE) {
            this.headingBuffer.shift();
        }
    }

    // Returns the circular mean of headingBuffer.
    // i.e averaging 350° and 10° should give 0°, not 180°.
    public getSmoothHeading(): number {

        // Edge case: if the buffer is empty, return 0
        if (this.headingBuffer.length === 0) return 0;

        // Convert headings to rad
        let sinSum = 0;
        let cosSum = 0;
        for (const k of this.headingBuffer) {
            sinSum += Math.sin(k * (Math.PI / 180));
            cosSum += Math.cos(k * (Math.PI / 180));
        }

        // calculate the mean angle in rad
        const meanRad = Math.atan2(sinSum / this.headingBuffer.length, cosSum / this.headingBuffer.length);
        let meanDeg = meanRad * (180 / Math.PI);

        // normalize the atan2 result to range (0-360)
        return (meanDeg + 360) % 360;

    }

    // UI calls this method to compare the smoothed heading vs target bearing
    public checkAlignment(targetDegrees: number | null): HeadingAlignmentResult{
        // always aligned unless the instruction has a target
        if (targetDegrees === null) {
            return { isAligned: true, degreesOff: 0, turnDirection: 'none' };
        }
        
        const smoothed = this.getSmoothHeading();

        const diff = ((targetDegrees - smoothed + 540) % 360) - 180;
        const absDiff = Math.abs(diff);

        // Pos diff means turn RIGHT
        // Neg diff means turn LEFT
        const turnDirection = absDiff <= this.ALIGNMENT_TOLERANCE_DEGREES
            ? 'none'
            : diff > 0 ? 'right' : 'left';
        
        return {
            isAligned: absDiff <= this.ALIGNMENT_TOLERANCE_DEGREES,
            degreesOff: Math.round(absDiff),
            turnDirection,
        };
